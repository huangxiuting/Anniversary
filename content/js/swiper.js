...
;+(function($) {
  ...
  $.fn.swiper = function(params) {
    return this.each(function() {
      ...
      if (!swiper)
        $this.data('swiper', new Swiper(this, $.extend({}, defaults, params)))
      return swiper
    })
  }
  ...
  Photos.prototype = {
    ...
    open: function(index) {
      ...
      if (!this.modal) {
        ...
        this.wrapper.transform(
          'translate3d(-' +
            $(window).width() * this.config.initIndex +
            'px,0,0)'
        )
        this.container
          .find('.caption-item')
          .eq(this.config.initIndex)
          .addClass('active')
        this.container
      ...
      this.container
        .addClass('swiper-container-visible')
    ...
    onTouchMove: function(e) {
      ...
      if (this.currentScale === 1 || this.overflow) {
        ...
        if (this.activeIndex === 0 && this.wrapperDiff > 0)
          this.wrapperDiff = Math.pow(this.wrapperDiff, 0.8)
        if (
          this.activeIndex === this.config.items.length - 1 &&
          this.wrapperDiff < 0
        )
          this.wrapperDiff = -Math.pow(-this.wrapperDiff, 0.8)
    ...
    onTouchEnd: function(e) {
      ...
      if (
        duration < 200 &&
        (!this.touchMove ||
          (Math.abs(this.touchStart.x - this.touchMove.x) <= 2 &&
            Math.abs(this.touchStart.y - this.touchMove.y) <= 2))
      ) {
        this.onClick()
        return
      }
      if (this.wrapperDiff > 0) {
        if (
          this.wrapperDiff > this.containerWidth / 2 ||
          (this.wrapperDiff > 20 && duration < 300)
        ) {
          this.slidePrev()
        } else {
    ...
    onDoubleClick: function() {
      this.gestureImage = this.container
        .find('.swiper-slide')
        .eq(this.activeIndex)
        .find('img')
    ...
    adjust: function() {
      ...
      if (rect.height < this.containerHeight) {
        ...
        //如果比容器高，那么要保证上下不能有空隙
        if (rect.top > 0)
          this.imageTransform.y = this.imageTransform.y - rect.top
        else if (rect.bottom < this.containerHeight)
          this.imageTransform.y =
            this.imageTransform.y + this.containerHeight - rect.bottom
      }
